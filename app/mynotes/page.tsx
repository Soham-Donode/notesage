      ...
      <div className="min-h-screen bg-gradient-to-br from-background to-muted">
        <div className="container mx-auto px-4 py-6 sm:py-8">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 sm:mb-8 gap-4">
            <h1 className="text-2xl sm:text-3xl font-bold text-foreground">My Notes</h1>
            <button
              onClick={() => router.push('/generation')}
              className="flex items-center gap-2 bg-primary hover:bg-primary/90 text-primary-foreground px-3 sm:px-4 py-2 rounded-lg transition-colors hover-scale text-sm sm:text-base w-full sm:w-auto justify-center"
            >
          ...
          <div className="mb-4 sm:mb-6">
            <div className="relative max-w-md">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 border border-input bg-background rounded-lg focus:outline-none focus:ring-2 focus:ring-ring text-foreground placeholder-muted-foreground text-sm sm:text-base"
              />
            </div>
          </div>
          ...
          {userLoading || loading ? (
            ...
            <div className="text-center py-8 sm:py-12">
              ...
              {!searchQuery && (
                <button
                  onClick={() => router.push('/generation')}
                  className="bg-primary hover:bg-primary/90 text-primary-foreground px-4 sm:px-6 py-2 sm:py-3 rounded-lg transition-colors hover-scale text-sm sm:text-base"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              {filteredNotes.map((note) => (
                <div key={note._id} className="bg-card border border-border rounded-lg p-4 sm:p-6 hover:shadow-md transition-shadow hover-scale">
                  <div className="flex items-start justify-between mb-3">
                    <h3 className="font-semibold text-base sm:text-lg text-card-foreground line-clamp-2 pr-2">{note.title}</h3>
                  ...
                  <p className="text-xs sm:text-sm text-muted-foreground mb-2">Role: {note.role}</p>

                  <div className="text-xs sm:text-sm text-card-foreground mb-3 line-clamp-3">
